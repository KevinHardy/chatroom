...
app.controller('mainCtrl', function($scope, parseService, $interval){
  ...
  //here is code the instructor typed with us
...
/*  $scope.postData = function(message) {
  ...
  /*setInterval(function(){//can also use $interval, ensures this loop runs with "DigestLoop" in order to not cause problems
    $scope.getParseData();
  }, 1500)*/
});
